# EU4Corruptable
Eu4 in its most primal form, values are flipped and changed, events are added or modified, religions are broken, new government types, modified map, reworked trade, and reworked fun

If you want an entirely new eu4 experience with the same map and same history look no further, alot of anti-fun rules are removed or changed in ways you would not expect. However if you do want to expect the unexpected look at the list of changes below.

<details>
<summary>SPOILERS AHEAD, IF YOU WANT THE TRUE EXPERIENCE DON'T OPEN THIS MENU</summary>
  
# New government Reform's

1. Economy of conquest

- [ ] Just like the germans did it, now your overextension penalties are flipped! If you have uncored provinces you are able to continually improve this buff as you continue to conquor land, however you have to be careful to balance the land you do have cored as if you exceed more provinces uncored than cored you will begin to fire the disaster "The war to end all wars" which upon firing will send an event to your rivals to declare war on you at the same time. They won't be able to take land directly but will be able to release states, return cores, take money, pillage, and change religion.

2. Disarmed state

- [ ] This government reform will disable your army, you will not be able to summon mercenaries or deploy and land forces or navy. However you will benefit from having massively increased diplmoatic reputation, more diplo slots, development cost reduction fort maintnence reduction and your vassels will no longer gain desire from independence from army/navy size.

3. Anarchism

- [ ] This government reform doesnt let autonomy go below 90% however it gives massive buffs to manpower, army cost and production (Power to the workers). However you also cannot collect tax at all (taxation is theft). You won't have a ruler anymore and will instead start with "The will of the people", war exhaustion will now fire events that will lower your point generation from your leader as the people continue to get warn out from war. If you dont have any uncored provinces and your war exhaustion is at zero (You will get events gradually) you can eventually get back up to your ruler having 5/5/5 however this can also be buffed further with government reforms to either reduce the war exhaustion penalty or buff your leader past 6/6/6 in peace time based off prosperity. (Aragon starts as this btw).

# Adjusted Government Reform's

1. Defensive stance

- [ ] now you have to play extra defensive, Your armies suffer massive attrition enough so that a month tick will reduce your army to zero instantly. However in your own nation you don't suffer those penalties. Your enemies however do suffer from massive attrition penalties closer to finlands nation ideas.

2. Peasent Republic

- [ ] You literally do not have a ruler, no elections nothing. Just a 4 4 4 "Commune".

3. Prussia and Russia have swapped government mechanics
- [ ] 

5. Austrian Government
- [ ] Austria's government can now no longer declare war's, they get a bonus to their diplo rep and diplo slots and offer vasselization bonuses which will now be their only way to expand.

6. France now starts revolutionary (mentioned later).
- [ ] 


# Adjusted trade goods

1. Mamluks

- [ ] The mamluks will now only start with grain trade good provinces, expanding into other land now will trigger an event to change trade goods to grain with no other option

2. Gold inflation

- [ ] Gold will now cause 4x inflation, but developing gold provinces will cost half as much.

3. Fish and Jewish nations

- [ ] If you religion is jewish you will now gain a bonus to fish provinces doubling the trade goods value.

4. Copper prices

- [ ] Copper will cost half as much, Nassir's fault btw

# Religion

1. Hellenic

- [ ] Hellenic religion now exists in greece, Constantinople and Byzantium are still orthodox but athens and epirus start hellenic. Events will fire now if you occupy greece that will convert provinces in the region to that religion. If hellenic reaches 50% of your religious share the event will give you the option to convert. This will have the coptic system of buffs from the religion controlling certain provinces aswell as have the fetishest cult system.

2. Norse

- [x] Sweden, Denmark & Norway start norse now.

3. Celtic

- [ ] Ireland and Scotland are now celtic with similar government mechanics to Norse and having the ancestor mechanics aswell.

4. Catholocism and Reformation

- [ ] At the start of the game you can now swap to protestant, reformed or hussite. No more waiting for reformation.

- [x] All religions can now swap to all religions. No restrictions other than reforming to tribal religions including Celtic, Norse and Hellenic.

# States and Starting Map

1. The Baltics

- [x] Finland, Sapmi, Estonia & Danzig all start independent
- [x] Sweden and Denmark swap positions
- [x] Finland and Norway swap positions

2. The Rus

- [x] All Muscovite subjects start as independent.
- [x] Novgorod and Muscovy Swap places
- [x] Belarus exists.
- [x] Kiev and Galicia are both independent.
- [x] Theodoro and Crimea swap borders.
- [x] The great horde now begins as a merchant republic & Circassia is now a horde.

3. Most of Europe

- [x] Bohemia loses silesia. 
- [ ] Austria loses tyorl and styria. 
- [ ] Saxony loses thuringia. 
- [ ] Burgundy swaps places with its subjects and has the option to form the netherlands also starts as a peasents republic (Paris commune w/o Paris). 
- [ ] Switzwerland is now balkanized because cantons, ulm swaps places with holstein. 
- [ ] Bavaria starts centralized but their missions lead them to balkanize. 
- [ ] Brandenburg Swaps places with the teutons. 
- [ ] Stettin requires wolgast not existing and holding all of their cores to form pomerania and vice verse for wolgast. Wolgast now starts in tunisia and stettin starts in the isles (both still in HRE) 
- [x] Ireland starts in the HRE.
- [x] Scotland has cornish culture.
- [x] England has welsh culture.
- [x] Whales exists and has french culture.
- [x] Cornwall exists and has Breton culture.
- [x] Brittany has English culture.
- [x] France has Breton culture.
- [x] France is now even more balkanized. Now only occupies Paris with a heavy development cost reduction.
- [x] Castille is also now balkanized.
- [x] Granada now owns the entire south coast of Iberia.
- [x] Navara now controls all of the province bordering the Pyreneese mountains.
- [x] Provence now only has Anjou.
- [x] The papal states now start in Newfoundland.
- [x] Rome is uncolonized.
- [x] Naples now owns all of the aragonese land outside of iberia while the borders they had before are now occupied by tunis.
- [ ] Venizia is now indpendent.
- [x] Venice is only in regusa and regusa gets all of their land.
- [x] Nitra is independent.
- [ ] Mazovia is independent.
- [x] Krakow is independent.
- [ ] Berlin is a city state.

4. Balkans

- [ ] Serbia is now a city state in belgrad.
- [ ] North Macedonia exists.
- [ ] Kosovo exists.
- [x] Croatia starts free.
- [x] Bosnia and Herzogovina swap places.
- [x] Bulgaria is independent and is an orthodox horde.
- [ ] All ottoman balkan lands now become city states (outside of the formerly mentioned).
- [x] Wallachia and Moldova swap places and the event to become a subject of hungary or poland no longer fires.
- [x] Transylvania exists.
- [ ] Cilli is now slovenia.
- [x] All greek venetian and genoan lands are uncolonized.
- [ ] The knights and the Ottomans swap places.
- [ ] Yugoslavia is now a formable.
- [ ] All Balkan mountains and Hills get turned into farmland.
- [ ] All Balkan Farmland and Plains get turned into steppes.
- [x] Every Balkan province starts with a fort.
- [ ] Hagia Sophia and The Parthenon can now be moved.
- [x] Albania now controls all of the papal states lands that they had (aside from rome)
- [x] Albnias land is now owned by athens
- [ ] Athens starts independent.
- [x] Greece is independent.

5. The Middle east and North Africa

- [x] Trebizond and the Timurids swap places. Karaman, Candar, Dulkair, Hisn Kayfa and Bitlis swap places with the timurid vassals.
- [ ] Syria and Qara Qoyunlu swap places.
- [x] Tunisia starts uncolonized now.
- [x] Morocco is now just Sus.
- [ ] All Saudi Nations swap places with native tribes (TBD WHICH TRIBES).
- [ ] Georgia swaps places with Karabakh.
- [ ] (more to come....)

6. America's

- [ ] All native tribes once a federation is formed can establish siberian frontiers.
- [ ] Vinland now exists at the start.
- [ ] Quebec exists.
- [ ] Uraguay exists.
- [ ] Indianapolis, Memphis, Albuquerque, Fresno and Cleveland are starting city state.
- [ ] Wawa, Lehtbridge, Grand Prairie, Red Deer, Lloydminster and Brandon are starting city states.
- [x] The pope has touched down in newfoundland.
- [ ] Greenland is independent.
- [ ] Haiti is independent.
- [ ] Argentina and England split control of the Falklands.

7. East Asia & Oceania

- [ ] China starts with zero mandate and all of their bordering nations become hordes.
- [ ] Korea is now split in the middle with east and west korea.
- [ ] Japan's nation now exists in the Phillipines while the Phillipines are now in Kamchatka.
- [ ] Japan is now controlled by "The United States" with an immortal rule 3/1/6 MacArthur.
- [ ] Mongolia and Oirat swap places.
- [ ] The Jurchen tribes now all have farmland provinces and start with russian culture and Sikh religion.
- [ ] Taiwan exists in hong kong and hong kong control taiwan and thailand
- [ ] Thailand now exists in papa new guinea.
- [ ] The entire tibet region is now a lake (You guys cant behave now no one gets you)
- [ ] Sri Lanka is now just "The Tamel Tigers".
- [ ] Nepals name is now changed to "The Thunder Dragon Empire"
- [ ] The Guangzhi Clique is a starting nation.
- [ ] (Not asia but) Hawaii is big.
- [ ] Australia and New Zealand swap places. Australia Shrinks and New Zealand Grows.
- [ ] Tasmania gets move to the middle of the black sea.


# There is going to be more to come this is just the stuff I want implemented before a first official launch
</details>
